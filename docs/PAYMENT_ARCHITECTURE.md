...
Analysed against upstream `HiEventsDev/Hi.Events` at commit `bad7f51` (2026-09-09, `VERSION` = `2.0.0-alpha.1`).
...
- **Endpoint:** `POST /public/webhooks/stripe` (`routes/api.php` line ~650) → `Http/Actions/Common/Webhooks/StripeIncomingWebhookAction`. No throttle middleware. Reads raw body + `Stripe-Signature`, **dispatches a queued closure** (`dispatch(static fn (IncomingWebhookHandler $h) => …)->catch(log)`), replies 204 immediately (400 only if dispatching itself throws). With `QUEUE_CONNECTION=sync` (default `.env.example`) it runs inline.
